...
test('ships one versioned schema and unique effect and sprite catalogs', async () => {
  ...
  assert.equal(contracts.effects.effects.length, 109);
  assert.equal(contracts.sprites.sprites.length, 44);
  assert.equal(new Set(contracts.effects.effects.map((effect) => effect.type)).size, 109);
  assert.equal(new Set(contracts.sprites.sprites.map((sprite) => sprite.id)).size, 44);
  assert.equal(contracts.recipeSchema.properties.primitives.items.oneOf.length, 109);
});
